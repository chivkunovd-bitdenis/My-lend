      ...
      <section className="px-5 py-16 md:px-10 md:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
            <div><div className="text-sm font-medium uppercase tracking-[0.24em] text-[#64748B]">Моя роль</div><h2 className="mt-4 text-3xl font-semibold tracking-tight text-[#0F172A] md:text-5xl">Я не классический РП, не чистый аналитик и не разработчик «по ТЗ».</h2><p className="mt-6 text-lg leading-8 text-[#475569]">Мой профиль — практическая автоматизация: разобраться в процессе, увидеть боль, придумать простое и рабочее решение и быстро собрать инструмент, который уберет рутину и ошибки.</p></div>
            <div className="space-y-6 text-lg leading-8 text-[#475569]"><div className="pt-2"><h3 className="text-2xl font-semibold tracking-tight text-[#0F172A]">Мой путь к решению всегда выглядит так:</h3><div className="mt-6 grid gap-4 sm:grid-cols-2">{approach.map((item) => { const Icon = item.icon; return <Card key={item.title} className="rounded-[2rem]"><CardContent className="p-6"><Icon className="mb-4 h-7 w-7 text-[#0F172A]" /><h4 className="text-lg font-semibold text-[#0F172A]">{item.title}</h4><p className="mt-3 text-base leading-7 text-[#475569]">{item.text}</p></CardContent></Card>; })}</div></div></div>
          </div>
        </div>
      </section>
